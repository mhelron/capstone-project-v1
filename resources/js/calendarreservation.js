...
document.getElementById('eventForm').addEventListener('submit', function(e) {
    ...
    // Validation check before AJAX request
    const menuField = document.getElementById('menu_name');
    const menuError = document.getElementById('menu_name_error'); // Get the error message element
    ...
    // Check if 'menu_name' is required and not filled
    if (menuField.required && !menuField.value) {
        menuError.style.display = 'block';
        menuError.textContent = "Menu is required."; // Show the error message
        return; // Prevent form submission if validation fails
    }

    // Show loading indication (optional)
    const submitButton = document.querySelector('button[type="submit"]');
    submitButton.setAttribute('disabled', true); // Disable the button to prevent multiple submissions
    submitButton.innerHTML = 'Submitting...'; // Change button text to show the form is being submitted

    let formData = new FormData(this); // Get the form data

    // Perform AJAX request
    fetch('{{ route("admin.calendar.reservation") }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': formData.get('_token'), // Pass the CSRF token from the form
        },
        body: formData,
    })
    ...
    .then(data => {
        ...
        if (data.success) {
            $('#eventModal').modal('hide'); // Close modal on success
            alert('Booking added successfully');
        } else {
            alert('Error: ' + data.message); // Show error message if failed
        }
    })
    .catch(error => {
    ...
    .finally(() => {
        ...
        submitButton.innerHTML = 'Add Booking'; // Reset the button text
    });
});
